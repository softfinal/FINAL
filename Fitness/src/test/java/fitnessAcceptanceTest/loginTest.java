...
public class loginTest {
    ...
    public void is_not_in_the_database(String string) {
        checkUserInDatabase(string, "the username is wrong");
    }

    @Then("Show {string} message.")
    public void show_message(String string) {
        logger.log(Level.INFO, String.format("Message: %s", string));
    }

    @Given("the {string} not in database")
    public void the_not_in_database(String string) {
        checkUserInDatabase(string, "the password is wrong");
    }
    ...
    // Helper method to check user data in the database and log message
    private void checkUserInDatabase(String userInput, String errorMessage) {
        if (!User.users1.equals(userInput)) {
            u.setLogstate(false);
            assertFalse(u.getLogstate());
            logger.log(Level.INFO, String.format("%s: %s", errorMessage, userInput));
        }
    }
